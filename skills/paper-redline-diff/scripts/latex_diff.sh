...
inject_custom_colors() {
    ...
    if [[ -n "$color_cmds" ]]; then
        # Insert after \begin{document} using a temp file so the script works on
        # both GNU sed and BSD sed (macOS).
        local tmpfile
        tmpfile=$(mktemp "${diff_file##*/}.XXXXXX")
        awk -v extra="$color_cmds" '
            { print }
            !done && /\\begin{document}/ {
                printf "%s", extra
                done = 1
